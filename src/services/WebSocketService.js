...
import { BackendStatus, SyncStatus, EntityTypeEnum, SyncOperationType } from '@/types';
...
import { mapNotificationDataToFrontendFormat } from '@/utils/fieldMapping';
...
let longTermReconnectTimer = null;
...
let currentRequestedTenantId = null; // Aktuell angefragter Tenant
...
export const WebSocketService = {
            ...
            socket.onmessage = async (event) => {
                ...
                try {
                    ...
                    if (message.type === 'status') {
                        const statusMessage = message; // Type assertion
                        ...
                        try {
                            ...
                            switch (updateMessage.entity_type) {
                                case EntityTypeEnum.ACCOUNT:
                                    const accountData = mappedData.single_entity;
                                    if (updateMessage.operation_type === SyncOperationType.CREATE) {
                                        await accountStore.addAccount(accountData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `Account ${accountData.id} created via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.UPDATE) {
                                        await accountStore.updateAccount(accountData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `Account ${accountData.id} updated via WebSocket.`);
                                    }
                                ...
                                case EntityTypeEnum.ACCOUNT_GROUP:
                                    const accountGroupData = mappedData.single_entity;
                                    if (updateMessage.operation_type === SyncOperationType.CREATE) {
                                        await accountStore.addAccountGroup(accountGroupData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `AccountGroup ${accountGroupData.id} created via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.UPDATE) {
                                        await accountStore.updateAccountGroup(accountGroupData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `AccountGroup ${accountGroupData.id} updated via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.DELETE) {
                                ...
                                case EntityTypeEnum.CATEGORY:
                                    const categoryData = mappedData.single_entity;
                                    if (updateMessage.operation_type === SyncOperationType.CREATE) {
                                        await categoryStore.addCategory(categoryData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `Category ${categoryData.id} created via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.UPDATE) {
                                        await categoryStore.updateCategory(categoryData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `Category ${categoryData.id} updated via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.DELETE) {
                                ...
                                case EntityTypeEnum.CATEGORY_GROUP:
                                    const categoryGroupData = mappedData.single_entity;
                                    if (updateMessage.operation_type === SyncOperationType.CREATE) {
                                        await categoryStore.addCategoryGroup(categoryGroupData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `CategoryGroup ${categoryGroupData.id} created via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.UPDATE) {
                                        await categoryStore.updateCategoryGroup(categoryGroupData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `CategoryGroup ${categoryGroupData.id} updated via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.DELETE) {
                                ...
                                case EntityTypeEnum.RECIPIENT:
                                    const recipientData = mappedData.single_entity;
                                    if (updateMessage.operation_type === SyncOperationType.CREATE) {
                                        await recipientStore.addRecipient(recipientData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `Recipient ${recipientData.id} created via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.UPDATE) {
                                        await recipientStore.updateRecipient(recipientData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `Recipient ${recipientData.id} updated via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.DELETE) {
                                ...
                                case EntityTypeEnum.TAG:
                                    const tagData = mappedData.single_entity;
                                    if (updateMessage.operation_type === SyncOperationType.CREATE) {
                                        await tagStore.addTag(tagData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `Tag ${tagData.name} created via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.UPDATE) {
                                        await tagStore.updateTag(tagData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `Tag ${tagData.name} updated via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.DELETE) {
                                ...
                                case EntityTypeEnum.RULE:
                                    const ruleData = mappedData.single_entity;
                                    if (updateMessage.operation_type === SyncOperationType.CREATE) {
                                        await ruleStore.addRule(ruleData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `AutomationRule ${ruleData.id} created via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.UPDATE) {
                                        await ruleStore.updateRule(ruleData.id, ruleData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `AutomationRule ${ruleData.id} updated via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.DELETE) {
                                ...
                                case EntityTypeEnum.PLANNING_TRANSACTION:
                                    const planningTransactionData = mappedData.single_entity;
                                    if (updateMessage.operation_type === SyncOperationType.CREATE) {
                                        await planningStore.addPlanningTransaction(planningTransactionData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `PlanningTransaction ${planningTransactionData.id} created via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.UPDATE) {
                                        await planningStore.updatePlanningTransaction(planningTransactionData.id, planningTransactionData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `PlanningTransaction ${planningTransactionData.id} updated via WebSocket.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.DELETE) {
                                ...
                                case EntityTypeEnum.TRANSACTION:
                                    ...
                                    const transactionData = mappedData.single_entity; // Using any for ExtendedTransaction compatibility
                                    if (updateMessage.operation_type === SyncOperationType.CREATE) {
                                        await transactionStore.addTransaction(transactionData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `Transaction ${transactionData.id} created via WebSocket with recipientId: ${transactionData.recipientId || 'none'}.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.UPDATE) {
                                        await transactionStore.updateTransaction(transactionData.id, transactionData, true); // true für 'fromSync'
                                        infoLog('[WebSocketService]', `Transaction ${transactionData.id} updated via WebSocket with recipientId: ${transactionData.recipientId || 'none'}.`);
                                    }
                                    else if (updateMessage.operation_type === SyncOperationType.DELETE) {
                        ...
                        const initialDataMessage = message; // Type assertion ist hier immer noch wichtig
    ...
    handleReconnection() {
        ...
        if (reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
            ...
            const currentInterval = Math.min(RECONNECT_INITIAL_INTERVAL * Math.pow(2, reconnectAttempts - 1), RECONNECT_MAX_INTERVAL);
        ...
        backendHealthCheckTimer = setInterval(async () => {
            ...
            try {
                ...
                warnLog('[WebSocketService]', 'Backend health check failed', {
                    ...
                    backendAvailable: false,
                    healthUrl: `${window.location.protocol}//${window.location.hostname}:${import.meta.env?.VITE_BACKEND_PORT || '8000'}/api/v1/websocket/health`,
    ...
    async processSyncQueue() {
        ...
        const activeDB = tenantStore.activeTenantDB;
        if (!activeDB) {
     ...
     */
    async processSyncChunk(entries) {
     ...
     */
    async waitForChunkAcknowledgment(entries) {
    ...
    initialize() {
        ...
        watch([() => webSocketStore.connectionStatus, () => webSocketStore.backendStatus], ([newConnectionStatus, newBackendStatus], [oldConnectionStatus, oldBackendStatus]) => {
    ...
    async processSyncAck(ackMessage) {
        try {
    ...
    async processSyncNack(nackMessage) {
        try {
            ...
            if (retryCount >= maxRetries) {
                ...
                const success = await tenantDbService.updateSyncQueueEntryStatus(nackMessage.id, SyncStatus.FAILED, `Failed after ${retryCount} retry attempts: ${nackMessage.reason} - ${nackMessage.detail || ''}`);
                if (success) {
                ...
                const success = await tenantDbService.updateSyncQueueEntryStatus(nackMessage.id, SyncStatus.PENDING, `Retry ${nextAttempt}/${maxRetries} scheduled after NACK: ${nackMessage.reason} - ${nackMessage.detail || ''}`);
                if (success) {
    ...
    setupQueueWatcher() {
        ...
        queueWatcher = watch(() => tenantStore.activeTenantId, async (tenantId) => {
            if (!tenantId)
                return;
    ...
    async startPeriodicSync(intervalMs = 60000) {
        if (autoSyncInterval) {
    ...
    setupConnectionWatcher() {
        const webSocketStore = useWebSocketStore();
        watch([() => webSocketStore.connectionStatus, () => webSocketStore.backendStatus], ([newConnStatus, newBackendStatus], [oldConnStatus, oldBackendStatus]) => {
    ...
    async requestServerDataStatus(tenantId) {
        if (!this.isOnlineAndReady()) {
    ...
    async handleDataStatusResponse(message) {
        const tenantStore = useTenantStore();
        const webSocketStore = useWebSocketStore();
     ...
     */
    async processTransactionsIntelligently(incomingTransactions, transactionStore) {
     ...
     */
    sendTenantDisconnect(tenantId, reason = 'user_logout') {
        if (!socket || socket.readyState !== WebSocket.OPEN) {
     ...
     */
    async processTenantDisconnectAck(ackMessage) {
