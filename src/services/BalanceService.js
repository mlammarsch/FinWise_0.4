...
class TransactionCache {
     ...
     */
    getOrCreateMonthCache(year, month) {
     ...
     */
    buildMonthCache(year, month, monthCache) {
        const transactionStore = useTransactionStore();
        const endDate = new Date(year, month + 1, 0);
        const endDateStr = toDateOnlyString(endDate);
        // Alle Transaktionen bis zum Monatsende
        const relevantTxs = transactionStore.transactions.filter(tx => toDateOnlyString(tx.date) <= endDateStr || toDateOnlyString(tx.valueDate) <= endDateStr);
     ...
     */
    getAccountTransactions(accountId, year, month) {
     ...
     */
    getCategoryTransactions(categoryId, year, month) {
        const monthCache = this.getOrCreateMonthCache(year, month);
...
class RunningBalanceQueue {
    constructor() {
        Object.defineProperty(this, "pendingAccounts", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Set()
        });
        Object.defineProperty(this, "accountDates", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
     ...
     */
    enqueueAccount(accountId, transactionDate) {
...
export const BalanceService = {
     ...
     */
    async calculateMonthlyBalancesIfNeeded() {
        ...
        if (latestBalance && latestBalance.lastCalculated) {
            const hasNewTransactions = transactionStore.transactions.some(tx => new Date(tx.updated_at || tx.createdAt || '1970-01-01') > new Date(latestBalance.lastCalculated));
     ...
     */
    async calculateMonthlyBalances() {
        await this.calculateAllMonthlyBalances();
    },
    /**
     ...
     */
    async updateMonthlyBalancesForChanges(updates) {
     ...
     */
    async calculateAllMonthlyBalances() {
        ...
        transactionStore.transactions.forEach(tx => {
            ...
            if (tx.valueDate)
                months.add(dayjs(tx.valueDate).format('YYYY-M'));
        });
   ...
   */
    calculateBalanceForMonth(year, month, changedAccountIds = [], changedCategoryIds = [], existingBalance) {
        ...
        const accountBalances = existingBalance?.accountBalances || {};
        ...
        const categoryBalances = existingBalance?.categoryBalances || {};
     ...
     */
    getTodayBalance(entityType, id, asOf = new Date()) {
        const mbStore = useMonthlyBalanceStore();
     ...
     * @param asOf - Stichtag (optional, default = heute)
     */
    getProjectedBalance(entityType, id, asOf = new Date()) {
        const mbStore = useMonthlyBalanceStore();
        const year = asOf.getFullYear();
        const month = asOf.getMonth();
        const mb = mbStore.getMonthlyBalance(year, month);
     ...
     */
    getRunningBalances(entityType, id, range, options = {}) {
        ...
        while (current.isSameOrBefore(end)) {
            ...
            if (options.includeProjection) {
                ...
                const planTxs = planStore.planningTransactions.filter(plan => {
                    if (!plan.isActive)
                        return false;
                    const occurrences = PlanningService.calculateNextOccurrences(plan, currentDateStr, currentDateStr);
     ...
     * @param options - Optionen wie includeProjection
     */
    getMonthlyBalances(entityType, id, range, options = { includeProjection: true }) {
        const mbStore = useMonthlyBalanceStore();
     ...
     * @param asOf - Stichtag
     */
    getPreviousPersistedBalance(entityType, id, asOf) {
        const mbStore = useMonthlyBalanceStore();
     ...
     */
    getAccountGroupBalance(groupId, asOf = new Date(), includeProjection = false) {
        const accountStore = useAccountStore();
        const groupAccounts = accountStore.accounts.filter(a => a.accountGroupId === groupId && a.isActive && !a.isOfflineBudget);
     ...
     */
    getTotalBalance(asOf = new Date(), includeProjection = false) {
        const accountStore = useAccountStore();
     ...
     */
    getCategoryWithChildrenBalance(categoryId, asOf = new Date(), includeProjection = false) {
        const categoryStore = useCategoryStore();
     ...
     */
    getTransactionsGroupedByDate(transactions, account) {
     ...
     */
    async recalculateRunningBalancesForAccount(accountId, fromDate) {
        ...
        const accountTransactions = txStore.transactions.filter(tx => tx.accountId === accountId && tx.type !== TransactionType.CATEGORYTRANSFER);
        ...
        if (fromDate) {
            ...
            const txsBeforeDate = sortedTransactions.filter(tx => toDateOnlyString(tx.date) <= dateStrBefore);
     ...
     */
    async recalculateAllRunningBalances() {
     ...
     */
    async batchUpdateRunningBalances(updates) {
        if (updates.length === 0) {
            return;
        }
        const txStore = useTransactionStore();
     ...
     */
    async batchUpdateRunningBalancesInDB(updates) {
     ...
     */
    enqueueRunningBalanceRecalculation(accountId, changedTransactionDate) {
     ...
     */
    async triggerRunningBalanceRecalculation(accountId, changedTransactionDate) {
     ...
     */
    async forceProcessRunningBalanceQueue() {
     ...
     */
    getTotalBudgetedForMonth(monthStart, monthEnd) {
        const categoryStore = useCategoryStore();
        const transactionStore = useTransactionStore();
