...
import { parseAmount, calculateStringSimilarity } from "@/utils/csvUtils";
...
export const useCSVImportService = defineStore('csvImportService', () => {
    ...
    const csvParseStatus = ref("idle");
    const error = ref("");
    const importStatus = ref("idle");
    // CSV-Daten
    const csvHeaders = ref([]);
    const allParsedData = ref([]);
    // Mappings
    ...
    function detectDelimiterAndDateFormat() {
        ...
        const effectiveDelimiter = configuration.value.delimiter === "custom" ? configuration.value.customDelimiter : configuration.value.delimiter;
        const rows = csvData.value.split(/\r?\n/).filter((row) => row.trim());
        if (rows.length < 2)
            return; // Brauchen mindestens Header + eine Datenzeile
    ...
    function identifyPossibleMappings() {
        if (allParsedData.value.length === 0 || csvHeaders.value.length === 0)
            return;
        ...
        csvHeaders.value.forEach((header) => {
            ...
            if (sampleValues.length === 0)
                return; // Keine Werte zum Analysieren
            // Analysiere die gesammelten Werte
            const hasDatePattern = sampleValues.some(value => /^\d{4}[-/\.]\d{1,2}[-/\.]\d{1,2}$/.test(value) || // YYYY-MM-DD, YYYY/MM/DD, YYYY.MM.DD
                /^\d{1,2}[-/\.]\d{1,2}[-/\.]\d{4}$/.test(value) || // DD-MM-YYYY, DD/MM/YYYY, DD.MM.YYYY
                /^\d{1,2}[-/\.]\d{1,2}[-/\.]\d{2}$/.test(value) // DD-MM-YY, DD/MM/YY, DD.MM.YY
            );
            const hasAmountPattern = sampleValues.some(value => /^-?\d+([,\.]\d+)?$/.test(value));
            const hasTextPattern = sampleValues.some(value => value.length > 0 &&
                !/^\d+([,\.]\d+)?$/.test(value) &&
        ...
        debugLog("CSVImportService", "Erkannte mögliche Mappings", JSON.stringify(possibleMappings.value));
    }
    /**
    ...
    function automaticMapping() {
        ...
        if (possibleMappings.value.date.length > 0) {
            ...
            const dateHeader = possibleMappings.value.date.find((h) => h.toLowerCase().includes("datum") ||
                h.toLowerCase().includes("date") ||
                h.toLowerCase().includes("valuta") ||
                h.toLowerCase().includes("wert")) || possibleMappings.value.date[0];
        ...
        if (possibleMappings.value.amount.length > 0) {
            ...
            const amountHeader = possibleMappings.value.amount.find((h) => h.toLowerCase().includes("betrag") ||
                h.toLowerCase().includes("summe") ||
                h.toLowerCase().includes("amount") ||
                h.toLowerCase().includes("geldeingang") ||
                h.toLowerCase().includes("geldausgang")) || possibleMappings.value.amount[0];
            mappedColumns.value.amount = amountHeader;
        }
        // Automatisches Mapping für Empfänger
        if (possibleMappings.value.recipient.length > 0) {
            const recipientHeader = possibleMappings.value.recipient.find((h) => h.toLowerCase().includes("empfänger") ||
                ...
                h.toLowerCase().includes("partner")) || possibleMappings.value.recipient[0];
            mappedColumns.value.recipient = recipientHeader;
        }
        // Automatisches Mapping für Notizen
        if (possibleMappings.value.notes.length > 0) {
            const notesHeader = possibleMappings.value.notes.find((h) => h.toLowerCase().includes("notiz") ||
                ...
                h.toLowerCase().includes("bemerkung")) || possibleMappings.value.notes[0];
        ...
        if (possibleMappings.value.category.length > 0 && !mappedColumns.value.category) {
            const categoryHeader = possibleMappings.value.category.find((h) => h.toLowerCase().includes("kat") ||
                h.toLowerCase().includes("kategorie") ||
                h.toLowerCase().includes("cat") ||
                h.toLowerCase().includes("category")) || possibleMappings.value.category[0];
    ...
    function parseDate(dateString) {
        ...
        try {
            ...
            debugLog("CSVImportService", "Error parsing date", JSON.stringify(error));
            return null;
        }
    }
    /**
     ...
     */
    function _extractTagsFromNotes(row) {
     ...
     */
    function findMatchingRecipient(row, searchText) {
        if (!searchText)
            return;
        // NEUE LOGIK: Prüfe zuerst ob searchText einem Kontonamen entspricht
        const matchingAccount = accountStore.accounts.find(account => account.name && account.name.toLowerCase() === searchText.toLowerCase().trim());
        if (matchingAccount) {
        ...
        const directMatch = recipientStore.recipients.find((r) => r.name && r.name.toLowerCase() === (searchText || '').toLowerCase().trim());
        if (directMatch) {
     ...
     */
    function findMatchingCategory(row, searchText) {
        if (!searchText)
            return;
        // Direktsuche nach exaktem Namen
        const directMatch = categoryStore.categories.find((c) => c.name && c.name.toLowerCase() === (searchText || '').toLowerCase());
        if (directMatch) {
    ...
    function identifyPotentialMerges() {
        if (!mappedColumns.value.date || !mappedColumns.value.amount)
            return;
        allParsedData.value.forEach((row) => {
            ...
            if (row._potentialAccountTransfer) {
                const accountTransferDuplicate = transactionStore.transactions.find((tx) => {
                    ...
                    const oppositeDirection = tx.accountId === row._potentialAccountTransfer.targetAccountId;
   ...
   */
    function applyRecipientToSimilarRows(row, recipientId) {
        if (!mappedColumns.value.recipient || !row) {
            debugLog("CSVImportService", "Abbruch: Keine Empfängerspalte oder ungültige Zeile", JSON.stringify({ recipientId }));
            return;
        }
        // Den ursprünglichen Wert in dieser Zeile ermitteln
        const originalValue = row[mappedColumns.value.recipient];
        if (!originalValue) {
            debugLog("CSVImportService", "Abbruch: Kein Wert in der Empfängerspalte gefunden");
            return;
        }
        ...
        const recipientName = recipientId
            ...
            : 'Kein Empfänger';
        debugLog("CSVImportService", `Wende Empfänger an: ${recipientId} (${recipientName}) auf CSV-Namen: "${originalValue}"`, JSON.stringify({ originalValue, recipientId, recipientName }));
        // Zähler für tatsächlich geänderte Zeilen
        let changedRowsCount = 0;
        let debugChangedRows = [];
        ...
        allParsedData.value.forEach((otherRow, index) => {
            ...
            if (otherValue === originalValue &&
                ...
                debugLog("CSVImportService", `Setze Empfänger ${recipientId} (${recipientName}) für Zeile mit CSV-Wert "${otherValue}"`, JSON.stringify({
        ...
        infoLog("CSVImportService", `Empfänger-Zuordnung abgeschlossen: ${changedRowsCount} Zeilen aktualisiert`, JSON.stringify({
            recipientId,
            recipientName,
            originalValue,
            changedRowsCount,
   ...
   */
    function applyCategoryToSimilarRows(row, categoryId) {
        if (!mappedColumns.value.category || !row) {
            debugLog("CSVImportService", "Abbruch: Keine Kategoriespalte oder ungültige Zeile", JSON.stringify({ categoryId }));
            return;
        }
        // Den ursprünglichen Wert in dieser Zeile ermitteln
        const originalValue = row[mappedColumns.value.category];
        if (!originalValue) {
            debugLog("CSVImportService", "Abbruch: Kein Wert in der Kategoriespalte gefunden");
            return;
        }
        ...
        const categoryName = categoryId
            ...
            : 'Keine Kategorie';
        debugLog("CSVImportService", `Wende Kategorie an: ${categoryId} (${categoryName}) auf CSV-Namen: "${originalValue}"`, JSON.stringify({ originalValue, categoryId, categoryName }));
        // Zähler für tatsächlich geänderte Zeilen
        let changedRowsCount = 0;
        let debugChangedRows = [];
        ...
        allParsedData.value.forEach((otherRow, index) => {
            ...
            if (otherValue === originalValue &&
                typeof otherValue === typeof originalValue &&
                String(otherValue).trim() === String(originalValue).trim()) {
                debugLog("CSVImportService", `Setze Kategorie ${categoryId} (${categoryName}) für Zeile mit CSV-Wert "${otherValue}"`, JSON.stringify({
                    originalIndex: otherRow._originalIndex,
                    csvValue: otherValue
                }));
        ...
        infoLog("CSVImportService", `Kategorie-Zuordnung abgeschlossen: ${changedRowsCount} Zeilen aktualisiert`, JSON.stringify({
            categoryId,
            categoryName,
            originalValue,
            changedRowsCount,
     ...
     */
    function prepareDataForImport(accountId) {
        const preparedData = [];
     ...
     */
    function isRowReadyForImport(row) {
        if (!row._selected)
            return false;
        const dateValue = mappedColumns.value.date
            ? parseDate(row[mappedColumns.value.date])
    ...
    function resolveRecipient(row) {
        ...
        if (!row.originalRecipientName)
            return null;
        // Fall 3: Automatische Anlage - Prüfe, ob der Name bereits existiert
        const existingRecipient = recipientStore.recipients.find(r => r.name && r.name.toLowerCase() === (row.originalRecipientName || '').toLowerCase());
     ...
     */
    function isDuplicateAccountTransfer(transfer, alreadyCreatedTransfers = []) {
        ...
        const existingTransfers = transactionStore.transactions.filter(tx => tx.type === TransactionType.ACCOUNTTRANSFER &&
     ...
     */
    function findPotentialDuplicates(targetAccountId) {
        const duplicates = [];
        for (let i = 0; i < allParsedData.value.length; i++) {
            ...
            const existingTransactions = transactionStore.transactions.filter(tx => {
                ...
                if (tx.type === TransactionType.ACCOUNTTRANSFER) {
                    ...
                    const matchingAccount = accountStore.accounts.find(account => account.name && account.name.toLowerCase() === payee.toLowerCase().trim());
                    if (matchingAccount) {
     ...
     */
    async function startImport(accountId) {
        importStatus.value = "importing";
        error.value = "";
        importedTransactions.value = [];
        // CSV-Import-spezifische Performance-Optimierungen aktivieren
        const originalSkipRunningBalanceRecalc = TransactionService._skipRunningBalanceRecalc;
        TransactionService._skipRunningBalanceRecalc = true;
        // Bulk-Import-Modus für weitere Optimierungen
        const originalBulkImportMode = globalThis.__FINWISE_BULK_IMPORT_MODE__;
        globalThis.__FINWISE_BULK_IMPORT_MODE__ = true;
        try {
            const preparedData = prepareDataForImport(accountId);
            debugLog("CSVImportService", "Starte Import von " + preparedData.length + " Transaktionen");
            // Map für bereits erstellte Empfänger, um Duplikate zu vermeiden
            const createdRecipients = new Map(); // recipientName -> recipientId
            // Tag-Verarbeitung: Sammle alle einzigartigen Tags und erstelle sie im Batch
            const allExtractedTags = new Set();
            const rowTagMapping = new Map(); // _uniqueRowIdentifier -> tagNames
            // Sammle alle extrahierten Tags aus den zu importierenden Zeilen
            for (const item of preparedData) {
                const originalRowData = allParsedData.value.find(row => row._uniqueRowIdentifier && item._uniqueRowIdentifier === row._uniqueRowIdentifier);
            ...
            const tagNameToIdMap = new Map(); // tagName (lowercase) -> tagId
            if (allExtractedTags.size > 0) {
                ...
                for (const tagName of allExtractedTags) {
                    ...
                    const existingTag = tagStore.tags.find(tag => tag.name.toLowerCase() === lowerTagName);
                    if (existingTag) {
            ...
            for (const item of preparedData) {
                ...
                const itemRowIdentifier = item._uniqueRowIdentifier;
                const extractedTagNames = rowTagMapping.get(itemRowIdentifier) || [];
                const extractedTagIds = extractedTagNames.map(tagName => tagNameToIdMap.get(tagName.toLowerCase())).filter(Boolean);
                ...
                transactionData._originalRowIdentifier = item._uniqueRowIdentifier;
            ...
            const accountTransfersToCreate = [];
            ...
            for (let i = 0; i < transactionsToImport.length; i++) {
                ...
                if (!tx.recipientId && !tx._skipAutoRecipientMatching && tx.payee) {
                    const matchingAccount = accountStore.accounts.find(account => account.name && account.name.toLowerCase() === tx.payee.toLowerCase().trim());
                    if (matchingAccount && matchingAccount.id !== tx.accountId) {
                        ...
                        tx._isAccountTransfer = true;
                        continue;
                    }
                }
                // Zusätzlich: Prüfe auf potentielle Account-Transfers aus findMatchingRecipient
                const originalRowData = allParsedData.value.find(row => row._uniqueRowIdentifier && tx._originalRowIdentifier === row._uniqueRowIdentifier);
                if (originalRowData && originalRowData._potentialAccountTransfer) {
                    const potentialTransfer = originalRowData._potentialAccountTransfer;
                ...
                if (!tx.recipientId && !tx._skipAutoRecipientMatching && tx.payee && !tx._isAccountTransfer) {
            ...
            if (accountTransfersToCreate.length > 0) {
                ...
                const duplicateTransferIndexes = new Set();
                ...
                for (let i = 0; i < transactionsToImport.length; i++) {
                    if (duplicateTransferIndexes.has(i)) {
                        transactionsToImport[i]._isDuplicateAccountTransfer = true;
                ...
                for (const transfer of validTransfers) {
                    try {
                        ...
                        const { fromTransaction, toTransaction } = await TransactionService.addAccountTransfer(transfer.fromAccountId, transfer.toAccountId, transfer.amount, // Bereits als positiver Betrag übergeben
                        transfer.date, transfer.valueDate, transfer.note, null, // planningTransactionId
            ...
            const filteredTransactionsToImport = transactionsToImport.filter(tx => !tx._isAccountTransfer && !tx._isDuplicateAccountTransfer);
            ...
            TransactionService._skipRunningBalanceRecalc = originalSkipRunningBalanceRecalc;
            globalThis.__FINWISE_BULK_IMPORT_MODE__ = originalBulkImportMode;
            ...
            const normalAccountIds = importedTransactions.value.map((tx) => tx.accountId).filter(Boolean);
            ...
            const transferAccountIds = importedTransactions.value
                .filter((tx) => tx.type === TransactionType.ACCOUNTTRANSFER)
                .flatMap((tx) => [tx.accountId, tx.transferToAccountId])
                .filter(Boolean);
            ...
            let oldestImportDate = undefined;
            ...
            try {
                ...
                BalanceService.calculateAllMonthlyBalances().catch((error) => {
            ...
            infoLog("CSVImportService", `${importedTransactions.value.length} Transaktionen erfolgreich importiert.`);
    ...
    const importSummary = computed(() => {
        ...
        const withRecipient = allParsedData.value.filter((row) => row._selected && row.recipientId).length;
        const withCategory = allParsedData.value.filter((row) => row._selected && row.categoryId).length;
        const potentialDuplicates = allParsedData.value.filter((row) => row._selected && row._potentialMerge).length;
        return {
            total,
            selected,
            ready,
     ...
     */
    function updateRowNote(rowIndex, newNote) {
    ...
    async function performRecipientMatching(recipientName) {
        if (!recipientName)
            return null;
        // Direkter Match (case-insensitive)
        const directMatch = recipientStore.recipients.find(r => r.name && r.name.toLowerCase() === recipientName.toLowerCase().trim());
        if (directMatch) {
    ...
    async function performCategoryMatching(categoryName) {
        if (!categoryName)
            return null;
        // Direkter Match (case-insensitive)
        const directMatch = categoryStore.categories.find(c => c.name && c.name.toLowerCase() === categoryName.toLowerCase().trim());
        if (directMatch) {
