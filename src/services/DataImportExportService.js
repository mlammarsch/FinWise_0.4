...
export class DataImportExportService {
     ...
     */
    static async exportDataAsJSON(dataType) {
        try {
     ...
     */
    static formatExportData(dataType, data) {
     ...
     */
    static downloadJSONFile(exportData, dataType) {
    ...
    static getAvailableDataTypes() {
        const dataTypes = ['rules', 'categories', 'recipients', 'tags', 'planning'];
     ...
     */
    static async importDataFromJSON(file, importMode) {
        try {
     ...
     */
    static validateImportData(fileContent) {
        let parsedData;
        try {
        ...
        const supportedTypes = ['rules', 'categories', 'recipients', 'tags', 'planning'];
     ...
     */
    static async performImport(importData, importMode) {
     ...
     */
    static async importRules(data, importMode) {
        const ruleStore = useRuleStore();
     ...
     * @param importMode - Import-Modus ('replace' oder 'merge')
     */
    static async importCategories(data, importMode) {
        const categoryStore = useCategoryStore();
        if (importMode === 'replace') {
     ...
     * @param importMode - Import-Modus ('replace' oder 'merge')
     */
    static async importRecipients(data, importMode) {
        const recipientStore = useRecipientStore();
        if (importMode === 'replace') {
     ...
     * @param importMode - Import-Modus ('replace' oder 'merge')
     */
    static async importTags(data, importMode) {
        const tagStore = useTagStore();
        if (importMode === 'replace') {
     ...
     * @param importMode - Import-Modus ('replace' oder 'merge')
     */
    static async importPlanning(data, importMode) {
        const planningStore = usePlanningStore();
        if (importMode === 'replace') {
     ...
     * @throws Error wenn der Export fehlschlägt
     */
    static async exportTenantDatabase() {
        try {
     ...
     * @throws Error wenn der Import fehlschlägt
     */
    static async importTenantDatabase(file, tenantName) {
        try {
