...
export const TransactionService = {
    ...
    resolvePayeeFromRecipient(recipientId, fallbackPayee) {
        if (recipientId) {
    ...
    assignDefaultCategoryIfNeeded(txData, isReconciliation = false, isIntentionalCategoryRemoval = false) {
    ...
    async addTransaction(txData, applyRules = true) {
    ...
    async addAccountTransfer(fromAccountId, toAccountId, amount, date, valueDate = null, note = '', planningTransactionId = null, recipientId) {
        if (fromAccountId === toAccountId)
            throw new Error('Quell = Zielkonto');
        if (amount === 0)
            throw new Error('Betrag 0');
    ...
    async updateAccountTransfer(id, updates) {
        const txStore = useTransactionStore();
        const original = txStore.getTransactionById(id);
        if (!original || original.type !== TransactionType.ACCOUNTTRANSFER || !original.counterTransactionId)
            return false;
        ...
        const originalUpdates = { amount: newAmount < 0 ? -absNewAmount : absNewAmount, accountId: toAccountId, date: dateUpdate, valueDate: valueDateUpdate, note: noteUpdate, recipientId: recipientIdUpdate, payee: recipientIdUpdate ? this.resolvePayeeFromRecipient(recipientIdUpdate) : `Transfer von ${fromName}`, transferToAccountId: fromAccountId };
        const counterUpdates = { amount: newAmount < 0 ? absNewAmount : -absNewAmount, accountId: fromAccountId, date: dateUpdate, valueDate: valueDateUpdate, note: noteUpdate, recipientId: recipientIdUpdate, payee: recipientIdUpdate ? this.resolvePayeeFromRecipient(recipientIdUpdate) : `Transfer zu ${toName}`, transferToAccountId: toAccountId };
    ...
    async addCategoryTransfer(fromCategoryId, toCategoryId, amount, date, note = '', recipientId) {
        ...
        const fromTx = { type: TransactionType.CATEGORYTRANSFER, date: normalizedDate, valueDate: normalizedDate, accountId: '', categoryId: fromCategoryId, amount: -Math.abs(amount), tagIds: [], payee: recipientId ? this.resolvePayeeFromRecipient(recipientId) : `Kategorientransfer zu ${toCategoryName}`, note, counterTransactionId: null, planningTransactionId: null, isReconciliation: false, isCategoryTransfer: true, toCategoryId: toCategoryId, reconciled: false, description: '', recipientId };
        const toTx = { ...fromTx, categoryId: toCategoryId, amount: Math.abs(amount), payee: recipientId ? this.resolvePayeeFromRecipient(recipientId) : `Kategorientransfer von ${fromCategoryName}`, toCategoryId: fromCategoryId };
    ...
    async addMultipleCategoryTransfers(transfers) {
    ...
    async updateCategoryTransfer(transactionId, gegentransactionId, fromCategoryId, toCategoryId, amount, date, note = undefined, recipientId) {
        const catStore = useCategoryStore();
    ...
    getCategoryTransferOptions(clickedCategoryId, isIncome) {
        ...
        const excludedIds = new Set([clickedCategoryId, ...getDescendants(clickedCategoryId)]);
        return allCategories.filter((cat) => !excludedIds.has(cat.id) && cat.isIncomeCategory === isIncome).sort((a, b) => a.name.localeCompare(b.name)).map((cat) => ({ label: cat.name, value: cat.id, }));
    },
    async updateCategoryTransferAmount(transactionId, newAmount) {
        const txStore = useTransactionStore();
        const original = txStore.getTransactionById(transactionId);
        if (!original || !original.isCategoryTransfer || !original.counterTransactionId)
            return;
        ...
        BalanceService.triggerMonthlyBalanceUpdate({ categoryIds: [original.categoryId, counter.categoryId].filter(Boolean), fromDate: original.date });
    },
    async addReconcileTransaction(accountId, amount, date, note) {
    ...
    async convertTransferToNormalTransaction(transactionId) {
        const txStore = useTransactionStore();
        const original = txStore.getTransactionById(transactionId);
        if (!original || !(original.type === TransactionType.ACCOUNTTRANSFER || original.type === TransactionType.CATEGORYTRANSFER))
            return null;
        if (original.counterTransactionId) {
            await txStore.deleteTransaction(original.counterTransactionId);
        }
        const updates = { type: original.amount > 0 ? TransactionType.INCOME : TransactionType.EXPENSE, counterTransactionId: null, isCategoryTransfer: false, transferToAccountId: undefined, toCategoryId: undefined };
        ...
        if (newTransaction) {
            BalanceService.triggerMonthlyBalanceUpdate({ accountIds: [original.accountId, newTransaction.accountId].filter(Boolean), categoryIds: [original.categoryId, newTransaction.categoryId].filter(Boolean), fromDate: original.date });
        }
    ...
    async deleteTransaction(id) {
        ...
        if (result) {
            ...
            const accountIds = [tx.accountId, counterTx?.accountId].filter(Boolean);
            const categoryIds = [tx.categoryId, counterTx?.categoryId].filter(Boolean);
    ...
    async bulkDeleteTransactions(ids) {
        ...
        ids.forEach(id => {
            ...
            if (tx?.counterTransactionId)
                allIdsToDelete.add(tx.counterTransactionId);
            if (tx)
    ...
    async bulkAssignAccount(transactionIds, newAccountId) {
        const txStore = useTransactionStore();
        let updatedCount = 0;
        const errors = [];
        ...
        if (updatedTransactions.length > 0) {
            const accountIds = new Set(updatedTransactions.map(t => t.accountId).filter(Boolean));
    ...
    async bulkAssignCategory(transactionIds, newCategoryId) {
        const txStore = useTransactionStore();
        let updatedCount = 0;
        const errors = [];
        const updatedTransactions = [];
        ...
        if (updatedTransactions.length > 0) {
            const accountIds = new Set(updatedTransactions.map(t => t.accountId).filter(Boolean));
            const categoryIds = new Set(updatedTransactions.map(t => t.categoryId).filter(Boolean));
            const fromDate = updatedTransactions.sort((a, b) => a.date.localeCompare(b.date))[0].date;
            BalanceService.triggerMonthlyBalanceUpdate({ accountIds: Array.from(accountIds), categoryIds: Array.from(categoryIds), fromDate });
        }
        return { success: errors.length === 0, updatedCount, errors };
    },
    async bulkChangeRecipient(transactionIds, newRecipientId, removeAll = false) {
        let updatedCount = 0;
        const errors = [];
        ...
        for (const id of new Set(transactionIds)) {
            ...
            if (tx) {
                const updateData = { recipientId: removeAll ? undefined : newRecipientId || undefined, payee: removeAll ? '' : recipientName, };
    ...
    async bulkAssignTags(transactionIds, tagIds, removeAll = false) {
        let updatedCount = 0;
        const errors = [];
        const updatedTransactions = [];
        for (const id of new Set(transactionIds)) {
            const tx = this.getTransactionById(id);
            if (tx) {
                const updateData = { tagIds: removeAll ? [] : tagIds || [] };
                const success = await this.updateTransaction(id, updateData);
                if (success) {
                    updatedCount++;
                    const updatedTx = this.getTransactionById(id);
    ...
    getIncomeExpenseSummary(startDate, endDate) {
        ...
        transactions.forEach(tx => {
            if (tx.type === TransactionType.ACCOUNTTRANSFER || tx.isCategoryTransfer)
                return;
     ...
     */
    async processTransactionsIntelligently(incomingTransactions) {
     ...
     */
    async applyPreAndDefaultRulesToTransactions(transactions) {
        const ruleStore = useRuleStore();
     ...
     */
    async applyPostStageRulesToTransactions(transactionIds) {
        const ruleStore = useRuleStore();
        const txStore = useTransactionStore();
     ...
     */
    async bulkChangeDate(transactionIds, newDate) {
        const txStore = useTransactionStore();
        let updatedCount = 0;
        const errors = [];
     ...
     */
    async bulkSetReconciled(transactionIds) {
        const txStore = useTransactionStore();
        let updatedCount = 0;
        const errors = [];
        for (const id of transactionIds) {
     ...
     */
    async bulkRemoveReconciled(transactionIds) {
        const txStore = useTransactionStore();
        let updatedCount = 0;
        const errors = [];
        for (const id of transactionIds) {
     ...
     */
    async updateRecipientReferences(sourceRecipientIds, newRecipientId) {
        const txStore = useTransactionStore();
     ...
     */
    async validateRecipientDeletion(recipientIds) {
        const txStore = useTransactionStore();
