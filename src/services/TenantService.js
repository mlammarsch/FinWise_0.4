...
export const TenantService = {
    ...
    async renameTenant(tenantId, newName) {
        try {
            const session = useSessionStore();
            if (!session.currentUserId) {
                throw new Error('Kein eingeloggter User');
    ...
    ensureTenantSelected() {
        ...
        if (!ok)
            debugLog('[TenantService]', 'Kein Tenant aktiv – Auswahl erforderlich');
        return ok;
    },
    /**
     ...
     */
    async deleteTenantCompletely(tenantId, router) {
        try {
            const session = useSessionStore();
     ...
     */
    async resetTenantDatabase(tenantId, router) {
        try {
     ...
     */
    async resetTenantToInitialState(tenantId, router) {
        try {
            const session = useSessionStore();
            const tenantStore = useTenantStore();
            if (!session.currentUserId) {
