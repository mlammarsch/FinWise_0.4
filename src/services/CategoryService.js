...
export const CategoryService = {
     ...
     */
    expandCategoryGroupsBatch(groupIds) {
     ...
     */
    async updateCategoryGroup(id, updates) {
        if (!id) {
     ...
     */
    async updateCategoryBalance(id, amount) {
        if (!id) {
     ...
     */
    async updateCategoriesWithSortOrder(categoryUpdates) {
        try {
            const categoryStore = useCategoryStore();
            let successCount = 0;
     ...
     */
    async updateCategoryGroupsWithSortOrder(groupUpdates) {
        try {
            const categoryStore = useCategoryStore();
            let successCount = 0;
     ...
     */
    calculateCategorySortOrder(groupId, newOrder) {
        const updates = [];
     ...
     */
    calculateCategoryGroupSortOrder(newOrder, isIncomeGroups) {
        const updates = [];
        // Detailliertes Logging für Debugging
