...
export async function testCategoryTransferOnIncomeChange(): Promise<void> {
  try {
    ...
    const categoryTransfersBeforeUpdate = transactionStore.transactions.filter(
    ...
    const categoryTransfersAfterUpdate = transactionStore.transactions.filter(
      tx => tx.type === TransactionType.CATEGORYTRANSFER
    ).length;
    ...
    const newCategoryTransfers = transactionStore.transactions.filter(
      tx => tx.type === TransactionType.CATEGORYTRANSFER &&
...
if (typeof window !== 'undefined') {
  (window as any).testCategoryTransferOnIncomeChange = testCategoryTransferOnIncomeChange;
}
