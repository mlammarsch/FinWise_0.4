import { RecurrencePattern, TransactionType, AmountType, RecurrenceEndType, WeekendHandlingType, RuleConditionType, RuleActionType } from '../types';
import { debugLog } from '@/utils/logger';
/**
 * Formatiert Formulardaten für die Speicherung als Planungstransaktion
 */
export function formatTransactionForSave(formData) {
    // Effektiver Betrag basierend auf Transaktionstyp
    let effectiveAmount = formData.amount;
    const isAccountOrCategoryTransfer = formData.transactionType === TransactionType.ACCOUNTTRANSFER ||
    ...
    const transactionData = {
        ...
        recipientId: isAccountOrCategoryTransfer
            ? null
            : formData.recipientId,
        amount: effectiveAmount,
        amountType: formData.amountType,
        approximateAmount: formData.amountType === AmountType.APPROXIMATE
            ? formData.approximateAmount
            : undefined,
        minAmount: formData.amountType === AmountType.RANGE ? formData.minAmount : undefined,
        maxAmount: formData.amountType === AmountType.RANGE ? formData.maxAmount : undefined,
        note: formData.note,
        startDate: formData.startDate,
        valueDate: formData.valueDate,
        endDate: effectiveRecurrenceEndType === RecurrenceEndType.DATE
            ? formData.endDate
            : null,
        recurrencePattern: effectiveRecurrencePattern,
        recurrenceEndType: effectiveRecurrenceEndType,
        recurrenceCount: effectiveRecurrenceEndType === RecurrenceEndType.COUNT
            ? formData.recurrenceCount
            : null,
        executionDay: effectiveRecurrencePattern === RecurrencePattern.MONTHLY &&
...
export function getInitialRuleValues(params) {
    ...
    return {
        ...
        description: `Automatisch erstellt für Planungstransaktion "${name || recipientName}"`,
        stage: "DEFAULT",
        isActive: true,
        conditions: [
            {
