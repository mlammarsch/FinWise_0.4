 ...
 */
export function convertDateToIso(dateString, format = "YYYY-MM-DD") {
    if (!dateString)
        return null;
    try {
...
export function createRecurrenceDescription(params) {
    const { startDate, repeatsEnabled, recurrencePattern, executionDay, moveScheduleEnabled, weekendHandlingDirection } = params;
    if (!repeatsEnabled) {
...
export function calculateUpcomingDates(params) {
    const { startDate, repeatsEnabled, recurrencePattern, recurrenceEndType, endDate, recurrenceCount, executionDay, moveScheduleEnabled, weekendHandlingDirection } = params;
    const upcomingDates = [];
    if (!startDate)
