...
export const useTransactionStore = defineStore('transaction', () => {
    ...
    /* --------------------------------------------------- Getters */
    const getTransactionById = computed(() => (id) => transactions.value.find(t => t.id === id));
    const getTransactionsByAccount = computed(() => (accountId) => transactions.value
        .filter(t => t.accountId === accountId)
        .sort((a, b) => b.date.localeCompare(a.date)));
    const getTransactionsByCategory = computed(() => (categoryId) => transactions.value
    ...
    async function addTransaction(tx, fromSync = false) {
     ...
     */
    async function addMultipleTransactions(txs, fromSync = false) {
        if (txs.length === 0) {
    ...
    async function updateTransaction(id, updates, fromSync = false) {
        ...
        const now = new Date().toISOString();
        const transactionUpdatesWithTimestamp = {
            ...updates,
        ...
        if (fromSync) {
            ...
            if (!localTransaction) {
                ...
                const fullTransaction = { id, ...transactionUpdatesWithTimestamp };
    ...
    async function deleteTransaction(id, fromSync = false) {
