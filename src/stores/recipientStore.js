...
(function (ErrorSeverity) {
    ...
    ErrorSeverity["CRITICAL"] = "critical"; // Critical error, full rollback required
})(ErrorSeverity || (ErrorSeverity = {}));
export const useRecipientStore = defineStore('recipient', () => {
    const tenantDbService = new TenantDbService();
    const recipients = ref([]);
    // Rollback-Management State
    const rollbackActions = ref(new Map());
    const stateSnapshots = ref(new Map());
    const enhancedErrors = ref(new Map());
    /**
     * Erstellt einen State Snapshot für Rollback-Operationen
     * Task 5.6: State Snapshot-System
     */
    function createStateSnapshot(operationName, operationId, affectedRecipientIds) {
        const snapshot = {
            id: uuidv4(),
            timestamp: Date.now(),
     ...
     * Task 5.6: State Snapshot-System
     */
    async function restoreFromSnapshot(operationId) {
     ...
     */
    function addRollbackAction(operationId, action) {
     ...
     */
    function cleanupRollbackData(operationId) {
     ...
     */
    function addEnhancedError(operationId, error) {
        const enhancedError = {
            ...error,
            id: uuidv4(),
            timestamp: Date.now()
     ...
     */
    async function executeWithRollback(operation, rollbackActions, operationName) {
    ...
    const getRecipientById = computed(() => (id) => recipients.value.find(r => r.id === id));
    async function addRecipient(recipientData, fromSync = false) {
        ...
        if (fromSync) {
            ...
            if (localRecipient) {
                ...
                const localTimestamp = localRecipient.updatedAt || localRecipient.updated_at;
                const incomingTimestamp = recipientWithTimestamp.updatedAt || recipientWithTimestamp.updated_at;
        ...
        if (existingRecipientIndex === -1) {
            ...
            const storeTimestamp = recipients.value[existingRecipientIndex].updatedAt || recipients.value[existingRecipientIndex].updated_at;
            const incomingTimestamp = recipientWithTimestamp.updatedAt || recipientWithTimestamp.updated_at;
     ...
     */
    async function addMultipleRecipients(recipientsToAdd, fromSync = false) {
            ...
            if (fromSync) {
                ...
                for (const recipient of recipientsWithTimestamp) {
                    ...
                    if (existingIndex === -1) {
                        ...
                        const storeTimestamp = recipients.value[existingIndex].updatedAt || recipients.value[existingIndex].updated_at;
                        const incomingTimestamp = recipient.updatedAt || recipient.updated_at;
    ...
    async function updateRecipient(recipientUpdatesData, fromSync = false) {
        const recipientUpdatesWithTimestamp = {
        ...
        if (fromSync) {
            ...
            const localTimestamp = localRecipient.updatedAt || localRecipient.updated_at;
            const incomingTimestamp = recipientUpdatesWithTimestamp.updatedAt || recipientUpdatesWithTimestamp.updated_at;
        ...
        if (idx !== -1) {
            ...
            const storeTimestamp = recipients.value[idx].updatedAt || recipients.value[idx].updated_at;
            const incomingTimestamp = recipientUpdatesWithTimestamp.updatedAt || recipientUpdatesWithTimestamp.updated_at;
            if (!fromSync || (incomingTimestamp && (!storeTimestamp || new Date(incomingTimestamp) > new Date(storeTimestamp)))) {
    ...
    async function deleteRecipient(recipientId, fromSync = false) {
        ...
        if (!fromSync) {
            try {
                ...
                // Prüfe Transaktionen
                const transactionsWithRecipient = transactionStore.transactions.filter(tx => tx.recipientId === recipientId);
                ...
                const planningTransactionsWithRecipient = planningStore.planningTransactions.filter(ptx => ptx.recipientId === recipientId);
     ...
     */
    async function mergeRecipients(sourceRecipientIds, targetRecipient) {
        const operationId = uuidv4();
        const operationName = 'mergeRecipients';
        const errors = [];
        const enhancedErrorsList = [];
        ...
        try {
            ...
            try {
                ...
                const transactionsBefore = transactionStore.transactions.filter(t => t.recipientId && filteredSourceIds.includes(t.recipientId));
            ...
            try {
                ...
                const planningsBefore = planningStore.planningTransactions.filter(pt => pt.recipientId && filteredSourceIds.includes(pt.recipientId));
            ...
            try {
                ...
                const rulesBefore = ruleStore.rules.filter(rule => {
                    const hasRecipientCondition = rule.conditions.some(condition => (condition.type === 'RECIPIENT_EQUALS' || condition.type === 'RECIPIENT_CONTAINS') &&
                        typeof condition.value === 'string' && filteredSourceIds.includes(condition.value));
                    const hasRecipientAction = rule.actions.some(action => action.type === 'SET_RECIPIENT' &&
            ...
            let deletedCount = 0;
            const deletedRecipients = []; // Für Rollback
     ...
     */
    async function performPostMergeCleanup(mergedRecipientId, deletedRecipientIds) {
        const startTime = performance.now();
        ...
        try {
            ...
            const [{ useTransactionStore }, { usePlanningStore }, { useRuleStore }, { TransactionService }, { PlanningService }] = await Promise.all([
            ...
            const orphanedTransactions = transactionStore.transactions.filter(t => t.recipientId && deletedRecipientIds.includes(t.recipientId));
            ...
            const orphanedPlannings = planningStore.planningTransactions.filter(pt => pt.recipientId && deletedRecipientIds.includes(pt.recipientId));
            ...
            const orphanedRules = ruleStore.rules.filter(rule => {
                const hasOrphanedCondition = rule.conditions.some(condition => (condition.type === 'RECIPIENT_EQUALS' || condition.type === 'RECIPIENT_CONTAINS') &&
                    typeof condition.value === 'string' && deletedRecipientIds.includes(condition.value));
                const hasOrphanedAction = rule.actions.some(action => action.type === 'SET_RECIPIENT' &&
            ...
            const duplicatePlanningGroups = new Map();
            ...
            const remainingOrphanedTransactions = transactionStore.transactions.filter(t => t.recipientId && deletedRecipientIds.includes(t.recipientId)).length;
            const remainingOrphanedPlannings = planningStore.planningTransactions.filter(pt => pt.recipientId && deletedRecipientIds.includes(pt.recipientId)).length;
     ...
     */
    async function batchDeleteRecipients(recipientIds) {
        const operationId = uuidv4();
        ...
        try {
            ...
            let transactionValidation = null;
            let planningValidation = null;
            ...
            const combinedValidation = new Map();
     ...
     */
    async function createCoordinatedRollback(operationName, affectedRecipientIds, serviceOperations) {
        const operationId = uuidv4();
        const rollbackActions = [];
     ...
     */
    function registerServiceRollback(operationId, serviceName, rollbackAction, priority = 1) {
        addRollbackAction(operationId, {
     ...
     */
    async function executeCoordinatedRollback(operationId, serviceRollbacks) {
     ...
     */
    async function rollbackSyncQueueEntries(operationId, affectedEntityIds) {
        try {
            ...
            for (const entityId of affectedEntityIds) {
                try {
                    ...
                    await tenantDbService.addSyncQueueEntry({
                        entityType: EntityTypeEnum.RECIPIENT,
                        entityId,
                        operationType: 'DELETE', // Markiere für Rollback-Bereinigung
     ...
     */
    function createSyncQueueRollback(operationId, syncQueueEntries) {
        ...
        addRollbackAction(operationId, {
            ...
            execute: async () => {
                await rollbackSyncQueueEntries(operationId, syncQueueEntries.map(entry => entry.entityId));
            },
