...
import { RecurrencePattern, AmountType, RecurrenceEndType, WeekendHandlingType, SyncOperationType, } from '@/types';
dayjs.extend(isSameOrBefore);
dayjs.extend(weekday);
dayjs.extend(isSameOrAfter);
export const usePlanningStore = defineStore('planning', () => {
    /* ------------------------------------------------ State */
    const planningTransactions = ref([]);
    const transactionStore = useTransactionStore();
    const ruleStore = useRuleStore();
    const tenantDbService = new TenantDbService();
    /* ---------------------------------------------- Getters */
    const getPlanningTransactionById = computed(() => (id) => planningTransactions.value.find(tx => tx.id === id));
    const getUpcomingTransactions = computed(() => (days = 30, startDate) => {
     ...
     */
    function calculateNextOccurrences(planTx, startDate, endDate) {
        if (!planTx.isActive)
            return [];
     ...
     */
    function applyWeekendHandling(date, handling) {
        const day = date.day();
    ...
    /* ---------------------------------------------- Actions */
    async function addPlanningTransaction(p, fromSync = false) {
     ...
     */
    async function addMultiplePlanningTransactions(planningTransactionsToAdd, fromSync = false) {
        ...
        const processedPlanningTransactions = [];
        try {
    ...
    async function updatePlanningTransaction(id, upd, fromSync = false) {
    ...
    async function deletePlanningTransaction(id, fromSync = false) {
        try {
    ...
    async function loadPlanningTransactions() {
        try {
            ...
            if (dbTransactions.length > 0) {
                planningTransactions.value = dbTransactions.map((tx) => ({
                    ...tx,
            ...
            if (raw) {
                try {
                    const legacyTransactions = JSON.parse(raw).map((tx) => ({
                        ...tx,
                        name: tx.name || tx.payee || '',
                        date: toDateOnlyString(tx.startDate),
                        valueDate: toDateOnlyString(tx.valueDate || tx.startDate),
    ...
    /* ---------------------------------------- Sync-Handling */
    function handleSyncMessage(operation, planningTransaction) {
