...
export const useTransactionFilterStore = defineStore('transactionFilter', () => {
    ...
    const currentViewMode = ref("account");
    // Sortierung
    ...
    const filteredTransactions = computed(() => {
        ...
        if (selectedReconciledFilter.value) {
            txs = txs.filter((tx) => selectedReconciledFilter.value === "abgeglichen"
                ? tx.reconciled
                : !tx.reconciled);
        }
        // Filtern nach Tag
    ...
    const sortedCategoryTransactions = computed(() => {
        ...
        if (!sortKey.value) {
            ...
            return list.sort((a, b) => sortOrder.value === "asc"
