...
export const useAccountStore = defineStore('account', () => {
    ...
    const getAccountGroupById = computed(() => (id) => accountGroups.value.find(g => g.id === id));
    async function addAccount(accountData, fromSync = false) {
        // Bestimme sortOrder falls nicht gesetzt (Fallback-Mechanismus)
        let sortOrder = accountData.sortOrder;
        if (sortOrder === undefined && accountData.accountGroupId) {
            const accountsInGroup = accounts.value.filter(acc => acc.accountGroupId === accountData.accountGroupId);
            const maxSortOrder = accountsInGroup.reduce((max, acc) => Math.max(max, acc.sortOrder || 0), -1);
        ...
        if (!fromSync) {
            try {
                ...
                Object.keys(cleanAccountForSync).forEach(key => {
                    if (cleanAccountForSync[key] === undefined) {
                        delete cleanAccountForSync[key];
                    }
                });
    ...
    async function updateAccount(accountUpdatesData, fromSync = false) {
        const accountUpdatesWithTimestamp = {
            ...accountUpdatesData,
            // FE verwendet updatedAt; Mapping zu updated_at erfolgt bei SyncQueue
            updatedAt: accountUpdatesData.updatedAt || new Date().toISOString(),
        };
        if (fromSync) {
    ...
    async function deleteAccount(accountId, fromSync = false) {
    ...
    async function addAccountGroup(accountGroupData, fromSync = false) {
        let sortOrder = accountGroupData.sortOrder;
        if (sortOrder === undefined) {
            const maxSortOrder = accountGroups.value.reduce((max, g) => Math.max(max, g.sortOrder), -1);
            sortOrder = maxSortOrder + 1;
        }
        const accountGroupWithTimestamp = {
            ...accountGroupData,
            sortOrder,
            updatedAt: accountGroupData.updatedAt || new Date().toISOString(),
        };
        if (fromSync) {
        ...
        if (!fromSync) {
            try {
                ...
                if (!cleanGroupForSync.updatedAt) {
                    cleanGroupForSync.updatedAt = accountGroupWithTimestamp.updatedAt || new Date().toISOString();
                }
                // Entferne undefined-Werte
                Object.keys(cleanGroupForSync).forEach(key => {
                    if (cleanGroupForSync[key] === undefined) {
                        delete cleanGroupForSync[key];
                    }
                });
                await tenantDbService.addSyncQueueEntry({
    ...
    async function updateAccountGroup(accountGroupUpdatesData, fromSync = false) {
        const accountGroupUpdatesWithTimestamp = {
            ...accountGroupUpdatesData,
            updatedAt: accountGroupUpdatesData.updatedAt || new Date().toISOString(),
        };
        if (fromSync) {
        ...
        if (idx !== -1) {
            ...
            if (!fromSync) {
                try {
                    ...
                    // LWW: updatedAt sicherstellen (Mapping zu updated_at erfolgt später)
                    if (!cleanGroupForSync.updatedAt) {
                        cleanGroupForSync.updatedAt = accountGroupUpdatesWithTimestamp.updatedAt || new Date().toISOString();
                    }
                    // Entferne undefined-Werte
                    Object.keys(cleanGroupForSync).forEach(key => {
                        if (cleanGroupForSync[key] === undefined) {
    ...
    async function deleteAccountGroup(accountGroupId, fromSync = false) {
    ...
    async function updateAccountLogo(accountId, newLogoPath) {
    ...
    async function updateAccountGroupLogo(accountGroupId, newLogoPath) {
