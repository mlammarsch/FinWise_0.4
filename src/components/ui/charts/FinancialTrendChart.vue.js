...
const props = withDefaults(defineProps(), { showHeader: true });
...
const chartData = computed(() => {
    ...
    for (let i = 5; i >= 0; i--) {
        ...
        const monthTransactions = transactionStore.transactions.filter((tx) => {
            const txDate = dayjs(tx.date);
            return (txDate.isAfter(dayjs(startOfMonth).subtract(1, "day")) &&
...
watch(() => monthlyBalanceStore.monthlyBalances, () => {
    updateChart();
}, { deep: true });
...
const __VLS_withDefaultsArg = (function (t) { return t; })({ showHeader: true });
const __VLS_ctx = {};
