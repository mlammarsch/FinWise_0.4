...
const queueStatistics = ref(null);
const queueUpdateInterval = ref(null);
const isOnline = computed(() => webSocketStore.connectionStatus === WebSocketConnectionStatus.CONNECTED);
const isConnecting = computed(() => webSocketStore.connectionStatus === WebSocketConnectionStatus.CONNECTING);
...
function startSyncAnimation(minimumDuration = 3000) {
...
async function handleSyncButtonClick() {
    ...
    if (!isOnline.value || isManuallyProcessingSync.value) {
        warnLog("SyncButton", "Manual sync trigger ignored. Conditions not met (not online or already processing).", {
    ...
    try {
        ...
        if (accountStore.accounts && accountStore.accounts.length > 0) {
            infoLog("SyncButton", `Starting logo cache update for ${accountStore.accounts.length} accounts.`);
            for (const account of accountStore.accounts) {
                if (account.logo_path) {
                    ImageService.fetchAndCacheLogo(account.logo_path).catch((err) => {
                        errorLog("SyncButton", `Failed to fetch/cache logo for account ${account.id}: ${account.logo_path}`, err);
                    });
                }
            }
        }
        if (accountStore.accountGroups && // Use accountStore here
            accountStore.accountGroups.length > 0) {
            infoLog("SyncButton", `Starting logo cache update for ${accountStore.accountGroups.length} account groups.`);
            for (const accountGroup of accountStore.accountGroups) {
                // Use accountStore here
                if (accountGroup.logo_path) {
                    ImageService.fetchAndCacheLogo(accountGroup.logo_path).catch((err) => {
                        errorLog("SyncButton", `Failed to fetch/cache logo for account group ${accountGroup.id}: ${accountGroup.logo_path}`, err);
                    });
                }
            }
        }
...
// Watchers
watch(() => tenantStore.activeTenantId, async (newTenantId) => {
    if (newTenantId) {
        await updateQueueStatistics();
...
watch(() => webSocketStore.connectionStatus, (newStatus) => {
...
__VLS_asFunctionalElement(__VLS_intrinsicElements.div, __VLS_intrinsicElements.div)({
    ...{ class: "sync-button-container relative" },
});
__VLS_asFunctionalElement(__VLS_intrinsicElements.button, __VLS_intrinsicElements.button)({
    ...{ onClick: (__VLS_ctx.handleSyncButtonClick) },
    ...{ class: "btn btn-ghost btn-circle relative" },
    ...{ class: ({
            'animate-spin': __VLS_ctx.buttonState.animate,
            'btn-disabled': __VLS_ctx.isDisabled,
        }) },
    title: (__VLS_ctx.buttonState.title),
...
const __VLS_1 = __VLS_asFunctionalComponent(__VLS_0, new __VLS_0({
    icon: (__VLS_ctx.buttonState.icon),
    ...{ class: (__VLS_ctx.buttonState.iconColorClass) },
    ...{ style: {} },
}));
...
if (__VLS_ctx.queueStatistics && __VLS_ctx.queueStatistics.pendingCount > 0) {
    __VLS_asFunctionalElement(__VLS_intrinsicElements.div, __VLS_intrinsicElements.div)({
    ...
    (__VLS_ctx.queueStatistics.pendingCount > 99
        ? "99+"
        : __VLS_ctx.queueStatistics.pendingCount);
}
if (__VLS_ctx.syncAnimationState.isActive) {
    __VLS_asFunctionalElement(__VLS_intrinsicElements.div, __VLS_intrinsicElements.div)({
        ...{ class: "absolute inset-0 rounded-full border-2 border-warning border-t-transparent animate-spin" },
    });
}
/** @type {__VLS_StyleScopedClasses['sync-button-container']} */ ;
