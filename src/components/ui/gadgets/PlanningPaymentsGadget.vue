...
const upcomingPlanningTransactions = computed(() => {
  ...
  return upcomingTransactions.sort(
    (a, b) => dayjs(a.executionDate).valueOf() - dayjs(b.executionDate).valueOf()
  );
});
...
async function executePlanning(planningTransactionId: string, executionDate: string) {
  try {
...
async function skipPlanning(planningTransactionId: string, executionDate: string) {
  try {
...
function getTargetName(planning: PlanningTransaction): string {
  if (planning.transactionType === TransactionType.CATEGORYTRANSFER) {
    return categoryStore.getCategoryById(planning.transferToCategoryId || '')?.name || '-';
...
<template>
  <div>
    <div class="flex justify-between items-center mb-4">
      <div>
        <h3 v-if="showHeader" class="card-title text-lg">Geplante Zahlungen</h3>
      ...
      <div v-if="showActions" class="flex gap-2 items-center">
        <div class="flex gap-2">
          <button
            :class="planningLimit === 3 ? 'btn btn-xs btn-primary' : 'btn btn-xs btn-outline'"
            @click="setPlanningLimit(3)"
          >
            3
          </button>
          <button
            :class="planningLimit === 8 ? 'btn btn-xs btn-primary' : 'btn btn-xs btn-outline'"
        ...
        <button class="btn btn-xs btn-outline" @click="navigateToPlanning">Alle</button>
        ...
        >
          <Icon icon="mdi:play-circle" class="mr-1" />
          Fällige ({{ dueTransactions.length }})
        </button>
      </div>
    </div>

    <div v-if="upcomingPlanningTransactions.length > 0" class="space-y-1 mt-3">
      <div
        v-for="item in upcomingPlanningTransactions.slice(0, planningLimit)"
        :key="`${item.planningTransaction.id}-${item.executionDate}`"
        :class="[
          'flex items-center justify-between p-2 rounded-lg cursor-pointer transition-colors',
          item.isDue ? 'bg-info/10 hover:bg-info/20 border border-info/20' : 'bg-base-200 hover:bg-base-300',
        ]"
        ...
        <div class="flex items-center space-x-1 flex-1 min-w-0">
          <div class="flex-shrink-0">
            <span
              ...
              :class="getTransactionTypeClass(item.planningTransaction.transactionType)"
              :data-icon="getTransactionTypeIcon(item.planningTransaction.transactionType)"
            ></span>
          </div>
        ...
        <div class="flex items-center space-x-2 flex-shrink-0">
          <div class="text-right">
            <div
              ...
              :class="getTransactionTypeClass(item.planningTransaction.transactionType)"
            >
              <CurrencyDisplay :amount="item.planningTransaction.amount" :asInteger="true" />
            </div>
          </div>

          <div class="flex space-x-1">
            <div class="tooltip tooltip-top max-w-xs" data-tip="Planungstransaktion ausführen und echte Transaktion erstellen">
              <button
                class="btn btn-ghost btn-xs border-none"
                @click.stop="executePlanning(item.planningTransaction.id, item.executionDate)"
              >
                <Icon icon="mdi:play" class="text-base text-success" />
              </button>
            </div>
            <div class="tooltip tooltip-top max-w-xs" data-tip="Planungstransaktion überspringen (als erledigt markieren ohne Transaktion zu erstellen)">
              <button
                class="btn btn-ghost btn-xs border-none"
                @click.stop="skipPlanning(item.planningTransaction.id, item.executionDate)"
              >
                <Icon icon="mdi:skip-next" class="text-base text-warning" />
              </button>
            </div>
          </div>
        </div>
      ...
      <div v-if="upcomingPlanningTransactions.length > planningLimit" class="text-center pt-2">
