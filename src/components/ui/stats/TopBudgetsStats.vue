...
<template>
  <div>
    <div class="flex justify-between items-center mb-4">
      <div>
        <h3 v-if="showHeader" class="card-title text-lg">Top Budgets</h3>
        <p class="text-sm opacity-60">Aktueller Monat (MTD)</p>
      </div>
      <div v-if="showActions" class="flex gap-2 items-center">
        <div class="flex gap-2">
          <button
            :class="expensesLimit === 3 ? 'btn btn-xs btn-primary' : 'btn btn-xs btn-outline'"
            @click="setExpensesLimit(3)"
          >
            3
          </button>
          <button
            :class="expensesLimit === 5 ? 'btn btn-xs btn-primary' : 'btn btn-xs btn-outline'"
            @click="setExpensesLimit(5)"
          >
            5
          </button>
          <button
            :class="expensesLimit === 10 ? 'btn btn-xs btn-primary' : 'btn btn-xs btn-outline'"
      ...
      >
        <div class="flex justify-left items-center mb-2">
          ...
          <div class="text-right">
            <div class="text-sm font-semibold">
              <CurrencyDisplay :amount="expense.spent" :asInteger="true" />
            </div>
          </div>
        </div>
        ...
        <div class="relative">
          <div class="w-full bg-base-300 rounded-full h-1.5 relative overflow-hidden">
            <div
              :class="['h-full rounded-full transition-all duration-300', getExpenseBarColor(expense.spent, expense.budgeted)]"
              :style="{ width: getExpenseBarWidth(expense.spent, expense.budgeted) + '%' }"
            ></div>
          </div>
          ...
          <div
            ...
            :style="{ left: getBudgetMarkerPosition(expense.spent, expense.budgeted) + '%' }"
          >
            <div class="flex flex-col items-center">
              <div class="text-xs font-medium mb-0 bg-base-300 px-1 border border-neutral rounded">
                <CurrencyDisplay :amount="expense.budgeted" :asInteger="true" />
              </div>
        ...
        <div class="flex justify-between text-xs opacity-60 mt-1">
          ...
          <span v-if="expense.budgeted > 0 && expense.spent > expense.budgeted" class="text-error">
            +<CurrencyDisplay :amount="expense.spent - expense.budgeted" :asInteger="true" /> über Budget
          </span>
        </div>
      </div>
    </div>
