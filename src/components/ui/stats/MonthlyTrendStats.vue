  ...
  <div>
    ...
    <div class="overflow-x-auto">
      <table class="table table-zebra w-full table-compact">
        ...
        <tbody class="text-sm">
          <tr v-for="row in rows" :key="row.key" class="leading-tight">
            <td class="font-medium py-0">{{ row.monthLabel }}</td>

            <td class="text-right py-2 text-success">
              <CurrencyDisplay :amount="row.income" :asInteger="true" />
            </td>

            <td class="text-right py-1 text-error">
              <CurrencyDisplay :amount="row.expense" :asInteger="true" />
            </td>

            <td class="text-right py-1" :class="row.balance >= 0 ? 'text-success' : 'text-error'">
              <CurrencyDisplay :amount="row.balance" :asInteger="true" />
            </td>
            ...
            <td class="text-center py-1">
              ...
              >
                <span v-if="row.percent !== null">{{ row.arrow }} {{ formatPercent(row.percent) }}</span>
                <span v-else>—</span>
              </span>
            </td>
          </tr>
          ...
          <tr v-if="rows.length === 0">
            <td colspan="5" class="text-center py-4">Keine Daten vorhanden</td>
          </tr>
        </tbody>
      </table>
    </div>
