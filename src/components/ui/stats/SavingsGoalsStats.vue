...
<template>
  <div>
    <div class="flex justify-between items-center mb-4">
      <div>
        <h3 v-if="showHeader" class="card-title text-lg">Sparziele</h3>
        <p class="text-sm opacity-60">Fortschritt der Sparziele</p>
      </div>
      <div v-if="showActions" class="flex gap-2 items-center">
        <div class="flex gap-2">
          <button
            :class="savingsGoalsLimit === 3 ? 'btn btn-xs btn-primary' : 'btn btn-xs btn-outline'"
            @click="setSavingsGoalsLimit(3)"
          >
            3
          </button>
          <button
            :class="savingsGoalsLimit === 5 ? 'btn btn-xs btn-primary' : 'btn btn-xs btn-outline'"
            @click="setSavingsGoalsLimit(5)"
          >
            5
          </button>
          <button
            :class="savingsGoalsLimit === 10 ? 'btn btn-xs btn-primary' : 'btn btn-xs btn-outline'"
      ...
      >
        <div class="flex justify-left items-center mb-2">
          ...
          <div class="text-right">
            <div class="text-sm font-semibold">
              <CurrencyDisplay :amount="goal.targetAmount" :asInteger="true" />
            </div>
          </div>
        ...
        <div class="relative">
          <div class="w-full bg-base-300 rounded-full h-1.5 relative overflow-hidden">
            <div
              :class="['h-full rounded-full transition-all duration-300', goal.barColor]"
          ...
          >
            <div class="flex flex-col items-center">
              <div class="text-xs font-medium mb-0 bg-base-300 px-1 border border-neutral rounded">
                <CurrencyDisplay :amount="goal.currentAmount" :asInteger="true" />
              </div>
        ...
        <div class="flex justify-between text-xs opacity-60 mt-1">
          ...
          <span v-else class="text-info">
            Noch <CurrencyDisplay :amount="goal.targetAmount - goal.currentAmount" :asInteger="true" /> benötigt
          </span>
        </div>
      </div>
    </div>
